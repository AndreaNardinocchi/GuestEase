...
const AdminBookingModal: React.FC<BookingModalProps> = ({
   ...
   * When editing, we now pass the actual booking's UUID ignores the current booking
  ...
  // When editing, we check whether the room tied to this booking is still in the available list.
  const editRoomIsAvailable = isLoading
    ? true
    : !editingBooking
      ? true
      : (availableRooms?.some((r: any) => r.id === selectedRoom?.id) ?? false);

  // If the room is not available, we raise the overlapError
  const overlapError = !roomIsAvailable;

  const editOverlapError = isLoading ? false : !editRoomIsAvailable;
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            error={overlapError || editOverlapError || dateError}
            helperText={
              overlapError || editOverlapError
        ...
        <DialogActions>
          ...
          <Button
             ...
             */
            disabled={overlapError || editOverlapError || dateError}
            sx={{
